...
from server_evofed import Server
import time
...
def main():
    ...
    # --- EvoFed hyperparams ---
    master_seed = 123                    # controls theta_0 and per-round epsilon seeds
    num_clients = len(client_seeds)
    ppo_total_episodes = 60
    population_size = 128                # must be even (server enforces)
    num_rounds = 100
    sigma = 0.5
    ...
    server.train_evofed(log_path=log_path, save_path=save_path, load_path=load_path, load_models=load_models)
    print("EvoFed training complete. Final policy saved to:", save_path)
