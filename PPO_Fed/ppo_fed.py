...
class PPO:
    def __init__(self, state_dim: int, action_dim: int, env: gym.Env, client_id: int):
        ...
        self.episodes_per_batch = 10                                    # episodes per batch for training
        ...
        # ALG - Step 1:
        # PPO will receive them from the client
    ...
    def rollout(self, env_seed: int, round_idx: int, client_id: int):
        ...
        while episodes < self.episodes_per_batch:
            ...
            # Reset the environment - use seed only for the very first episode
            if self.total_episodes == 1 and env_seed is not None:
            ...
            for ep_t in range(self.max_timesteps_per_episode):
                ...
                try:
                    ...
                    print(f"    [Client {self.client_id}] ERROR in env.step at episode {episodes}, timestep {ep_t}: {e}", flush=True)
                    raise
    ...
    def learn(self, total_episodes, actor: torch.FloatTensor, critic: torch.FloatTensor, env_seed: int,
        ...
        # Load the parameters from the theta tensor
        ...
        while episodes_so_far < total_episodes:
            ...
            # print(f"    [Client {self.client_id}] Starting gradient updates for iteration {i_so_far}...", flush=True)
            ...
            # after the updates in range(self.n_updates_per_batch): loop finishes
            with torch.no_grad():
                _, curr_entropy = self.get_log_probability_using_actor(batch_obs, batch_acts)
            
            # print(f"    [Client {self.client_id}] Completed gradient updates for iteration {i_so_far}", flush=True)


        final_ppo_stats = {'reward': self.current_batch_average_raw_ep_return,
        ...
        upd_actor = self.actor.parameters_to_vector().clone().detach().requires_grad_(False)
        upd_critic = self.critic.parameters_to_vector().clone().detach().requires_grad_(False)
        return upd_actor, upd_critic, final_ppo_stats
